...
function loadPublishedEvaluations() {
    ...
    xhr.open('GET', '/api/view-published-evaluations', true);
...
function loadAnsweredEvaluations() {
    const xhr = new XMLHttpRequest();
    xhr.open('GET', '/api/view-answered-evaluations', true); // Path to the new PHP file
    xhr.onreadystatechange = function () {
        if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
...
function loadQuestions(evaluationID) {
    const xhr = new XMLHttpRequest();
    xhr.open('GET', `/api/get-evaluation-questions?evaluationID=${encodeURIComponent(evaluationID)}`, true);
    xhr.onreadystatechange = function () {
...
function displayEvaluationQuestions(evaluationID, evaluationName, startDate, endDate) {
    ...
    mainContent.innerHTML = `
        <div>
            ...
            <img alt="back" src="/assets/images/left-arrow-svgrepo-com.svg">
            </a>
            <div id="evaluationContent">
                ...
                <form id="submitAnswersForm">
                    ...
                    <button id="submitAnswersBtn" onclick="submitAnswers()" type="button">Submit</button>
                </form>
            </div>
        </div>
    `;
        ...
        if (xhr.readyState === XMLHttpRequest.DONE) {
            ...
            if (xhr.status === 200) {
                ...
                if (response.error) {
                    ...
                    answerXHR.open('GET', `/api/get-answers?evaluationID=${encodeURIComponent(evaluationID)}`, true);
...
function displayAnsweredEvaluationQuestions(evaluationID, evaluationName, startDate, endDate) {
    ...
    `;
    loadAllQuestionsWithAnswers(evaluationID);
}
...
function submitAnswers() {
    ...
    const xhr = new XMLHttpRequest();
    xhr.open('POST', '/api/submit-answers', true);
    xhr.onreadystatechange = function () {
        if (xhr.readyState === XMLHttpRequest.DONE) {
            if (xhr.status === 200) {
